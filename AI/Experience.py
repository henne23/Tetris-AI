...
class Experience:
    ...
    def getTrainInstance(self, modelLearn, modelDecide, batchSize):
        ...
        # hier wurde vorher über actions iteriert, geht es auch so?
        for index, _ in enumerate(rewards):
            if gameOvers[index]:
                outputs[index] = rewards[index]
            else:
                # 0,1 als Faktor aus KI-Modul ergänzt
                outputs[index] = rewards[index]*0.1 + self.discount * newOutputs[index]
